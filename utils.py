...
def sample_and_group_knn(xyz, points, npoint, nsample, use_xyz=True, idx=None):
    """
    Soft Attention Version of sample_and_group_knn
    """
    xyz_flipped = xyz.permute(0, 2, 1).contiguous()  # (B, N, 3)
    new_xyz = gather_operation(xyz, furthest_point_sample(xyz_flipped, npoint))  # (B, 3, npoint)

    if idx is None:
        idx = query_knn(nsample, xyz_flipped, new_xyz.permute(0, 2, 1).contiguous())

    grouped_xyz = grouping_operation(xyz, idx)  # (B, 3, npoint, nsample)
    relative_xyz = grouped_xyz - new_xyz.unsqueeze(3).repeat(1, 1, 1, nsample)  # (B, 3, npoint, nsample)

    if points is not None:
        grouped_points = grouping_operation(points, idx)  # (B, f, npoint, nsample)
        if use_xyz:
            new_points = torch.cat([relative_xyz, grouped_points], dim=1)  # (B, 3+f, npoint, nsample)
        else:
            new_points = grouped_points  # (B, f, npoint, nsample)
    else:
        new_points = relative_xyz  # (B, 3, npoint, nsample)

    # --- Compute Attention Weights ---
    # Use relative distance as attention score
    dist = torch.norm(relative_xyz, dim=1, keepdim=True)  # (B, 1, npoint, nsample)
    attention_scores = -dist  # closer points should have higher attention

    attention_weights = F.softmax(attention_scores, dim=-1)  # (B, 1, npoint, nsample)

    # --- Apply Attention Weights to Features ---
    new_points_weighted = new_points * attention_weights  # element-wise weighting

    return new_xyz, new_points_weighted, idx, relative_xyz


class MedoidGenerator(nn.Module):
    def __init__(self, npoint, nsample, in_channel, mlp, if_bn=True, group_all=False, use_xyz=True, if_idx=False):
        ...
        Args:
            ...
            mlp: list of int,
        """
        super(MedoidGenerator, self).__init__()
        self.npoint = npoint
        self.nsample = nsample
...
class MedoidTransformer(nn.Module):
    def __init__(self, in_channel, dim=256, n_knn=16, pos_hidden_dim=64, attn_hidden_multiplier=4):
        super(MedoidTransformer, self).__init__()
        self.n_knn = n_knn
...
class SSM1D(nn.Module):
    def __init__(self, dim, kernel_size=9, bidirectional=True):
        super().__init__()
        ...
        self.kernel = nn.Conv1d(dim, dim, kernel_size=kernel_size, padding=kernel_size // 2, groups=dim)
        self.linear = nn.Conv1d(dim, dim, 1)

    def forward(self, x):
        # x: (B, dim, N)
        gated = self.gate(x)
        filtered = self.kernel(gated * x)
        if self.bidirectional:
            reversed_x = torch.flip(x, dims=[-1])
            gated_rev = self.gate(reversed_x)
            filtered_rev = self.kernel(gated_rev * reversed_x)
            filtered = (filtered + torch.flip(filtered_rev, dims=[-1])) / 2
        return self.linear(filtered)


class SSM_Block(nn.Module):
    def __init__(self, in_channel, npoint, dim=256, n_knn=16, kernel_size=9, use_pos = True):
        super().__init__()
        self.n_knn = n_knn
        self.npoint = npoint
        self.use_pos = use_pos
        self.linear_start = nn.Conv1d(in_channel, dim, 1)
        self.local_ssm = SSM1D(dim+3, kernel_size)
        self.global_ssm = SSM1D(dim+3, kernel_size)
        self.linear_end = nn.Conv1d(dim+3, in_channel, 1)
    ...
    def forward(self, x, pos, idx=None):
        ...
        identity = x
        x = self.linear_start(x)  # (B, dim, N)
        k = self.n_knn

        new_pos, new_x, idx, grouped_xyz = sample_and_group_knn(pos, x, self.npoint, k, self.use_pos, idx=idx) # new_x: (B, C, N, k)
        B, C, N, _ = new_x.shape
        new_x = new_x.permute(0, 2, 1, 3)
        grouped_x = new_x.contiguous().view(B * N, C, k)  # (B*N, C, k)

        # Apply local SSM on each group
        grouped_x = self.local_ssm(grouped_x)  # (B*N, C, k)
        grouped_x = torch.max(grouped_x, dim=2)[0]  # (B*N, C)
        grouped_x = grouped_x.view(B, N, C).permute(0, 2, 1).contiguous()  # (B, C, N)

        # Apply global SSM on aggregated groups
        global_feat = self.global_ssm(grouped_x)  # (B, C, N)
        output = self.linear_end(global_feat) + identity  # Residual
        return output