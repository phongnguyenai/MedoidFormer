...
class FurthestSampling(Function):
    ...
    def forward(ctx, xyz, offset, new_offset):
        ...
        pointops_cuda.furthestsampling_cuda(b, n_max, xyz.float(), offset.int(), new_offset.int(), tmp, idx.int())
        del tmp
        return idx
...
class KNNQuery(Function):
    ...
    def forward(ctx, nsample, xyz, new_xyz, offset, new_offset):
        ...
        pointops_cuda.knnquery_cuda(m, nsample, xyz.float(), new_xyz.float(), offset.int(), new_offset.int(), idx.int(), dist2.float())
...
def queryandgroup(nsample, xyz, new_xyz, feat, idx, offset, new_offset, use_xyz=True):
    ...
    if new_xyz is None:
        new_xyz = xyz
    if idx is None:
        idx, _ = knnquery(nsample, xyz.float(), new_xyz.float(), offset.int(), new_offset.int()) # (m, nsample)

    n, m, c = xyz.shape[0], new_xyz.shape[0], feat.shape[1]
    grouped_xyz = xyz[idx.view(-1).int(), :].view(m, nsample, 3) # (m, nsample, 3)
    #grouped_xyz = grouping(xyz, idx) # (m, nsample, 3)
    grouped_xyz -= new_xyz.unsqueeze(1) # (m, nsample, 3)
    grouped_feat = feat[idx.view(-1).int(), :].view(m, nsample, c) # (m, nsample, c)
...
def interpolation(xyz, new_xyz, feat, offset, new_offset, k=3):
    ...
    """
    assert xyz.is_contiguous() and new_xyz.is_contiguous() and feat.is_contiguous()
    idx, dist = knnquery(k, xyz.float(), new_xyz.float(), offset, new_offset) # (n, 3), (n, 3)
    ...
    for i in range(k):
        new_feat += feat[idx[:, i].int(), :] * weight[:, i].unsqueeze(-1)
    return new_feat
...
class Interpolation(Function):
    ...
    def forward(ctx, xyz, new_xyz, input, offset, new_offset, k=3):
        ...
        idx, dist = knnquery(k, xyz.float(), new_xyz.float(), offset.int(), new_offset.int()) # (n, k), (n, k)
